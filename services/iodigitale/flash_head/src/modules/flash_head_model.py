...
try:
    ...
    # xFuser is Linux/multi-GPU only. Single-GPU runs never take the USP path,
    # so fall back to stubs that make `_maybe_enable_usp()` return False.
    XFUSER_AVAILABLE = False

    def _xfuser_missing(*args, **kwargs):
        raise ModuleNotFoundError("xfuser is not installed; sequence parallelism is unavailable")

    get_sequence_parallel_rank = _xfuser_missing
    get_sequence_parallel_world_size = _xfuser_missing
    get_sp_group = _xfuser_missing
    xFuserLongContextAttention = _xfuser_missing
