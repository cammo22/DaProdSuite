...
**Ultima pubblicata: 1.2.2.** Nove schede dentro la suite, un ambiente Python
...
| **LLaDA a 12 passi, non 50 (1.2.2)** | ⚠ **misurato, non dedotto**, col motore acceso su RTX 4060 8 GB a 1024: 8 passi 2,8 min, **12 passi 4,2 min**, 16 passi 5,3 min, 50 passi ~17 min (+50 s di caricamento). Venti secondi a passo perche' il trasformatore INT8 da 6,6 GB non ci sta in 8 e a CFG 5 la pipeline lavora su due latenti. Il tetto era 5 minuti: 12 passi ci stanno e l'immagine a 12 non si distingue da quella a 16. Massimo 24, non 50 |
| **LLaDA dice a che punto e' (1.2.2)** | ⚠ la 1.2.1 aveva messo la fase, ma con LLaDA non si muoveva lo stesso: quel pacco conta i passi con la barra di diffusers, che ComfyUI non ascolta. Agganciato `callback_on_step_end` **alla pipeline** (non al nodo: li' la firma e' fissa e solleva TypeError, che il ramo di riserva ingoiava in silenzio). **Provato**: 8 messaggi su 8 passi |
| **La Fila non pesa piu' (1.2.2)** | l'avanzamento della 1.2.1 aveva moltiplicato le spinte, e a ogni spinta si rileggeva tutto: cinque giri di rete per due pixel di barra. Adesso a ogni spinta solo `/macchina`, il resto ogni 1,5 s, e la fase si scrive in casa sulla pillola. Piu' la spazzata automatica: si tengono le ultime 100 righe **finite**, mai quelle vive, e i file non si toccano |
| **«Usa l'AI» tolto da tutti e tre i posti (1.2.2)** | era stato tolto da uno solo, ed e' il motivo per cui risultava ancora aperto. Le rotte restano: rimetterlo sara' riattaccare un bottone |
...
| **⚠ Ritirata la 1.1.1 (1.1.2)** | la sonda «risponde da Internet?» **non puo' riuscire da quella macchina**: Tailscale prende in mano il traffico verso i propri nodi di ingresso. Verificato con una sonda esterna vera che l'indirizzo risponde. La 1.1.1 metteva in fondo l'unico indirizzo che non scade e scriveva il falso nel pannello. Chi verifica e' chi ci deve arrivare |
...
| **LLaDA usa la scheda (1.0.9)** | il testo si codifica dove sta l'encoder, e quel pacco lascia in RAM encoder, queryformer, text_projection e sigvq. Spostati i primi tre (800 MB in tutto, i pesi GGUF non sono registrati): **da 94 a 33 secondi**. Il sigvq no: provato, stesso tempo e 2,6 GB di scheda in meno |
